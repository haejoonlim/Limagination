...
- [x] ~~신규 서식종 온도 내성~~ — 1·2차 34종 전부 완료 (소속 바이옴 band 내 정의 — 06 §3.5 검증 통과, 당시 107종 교차 0위반 — 현재 데이터 총계 103종).
...
> [!warning] 수치 정정 (2026-09-15 검수)
> 작성 시점 기준 수치 (107종 = 야생 35 + 전투 54 + 예정 18). 이후 데이터 정합 라운드에서 **몬스터 50종·야생 53종 확정** — 현재 도감 총계 **103종**. 하단 구 수치는 설계 당시 기록으로 유지.
...
- **바이옴 필터** — 선택 바이옴의 종 + 그 종들이 직접 먹는 근원자원만 표시(초원 9종 · 27간선). 전체 뷰는 복합 그물 1차(08 §8 T-08-1~4) 이후 103종 · 339간선이라 헤어변이므로 기본값 = 초원.
