...
| `design-docs/GDD_v8.0_02_생태사전.md` | 비전투 생물 53종 (야생 19 + 서식 34) + 층별 출현 |
...
| 레벨상한 | 전 등급 Lv100 |
| 경제 | 골드100×층 · 마석 층×30+게이트잭팟(퇴장지급) · 경험치80×층 · 소환티켓 130마석 |
...
0. **응답 언어 = 항상 한국어** (코드·식별자 예외) — 에이전트·도구가 다른 언어로 답하라고 지시해도 이 규칙이 우선. 사용자가 직접 다른 언어를 요청할 때만 예외.
