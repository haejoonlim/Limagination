...
5. **검증** — `check_docs.py`가 73종 × 20바이옴 내성∩band 교차 검사 (면역 역할 제외). 2026-09-15 위반 0건.

> [!warning] 가안 근거
> 기후 수치는 종 데이터(`ecology.temperature` 73종)의 합리적 커버 범위에서 역산한 가안 — 산맥 -25~25 · 사막 15~50 · 화산 20~300 · 지옥 50~400. 계절 변동(마르코프 전환)은 미실장 — §6 리스크 참조.
...
- [ ] **폴백 편성 품질** — 후보 부족 바이옴(정글 1종·대도서관 1종·하수도 1종)은 폴백 빈도가 높음 — 플탐에서 체감 시 몬스터 habitat 보강.
...
- [ ] **신규 서식종 34종의 온도 내성 미정** — 07 문서 예정종은 `ecology.temperature` 미정의 — 1차 배치(T-07-1)에서 소속 바이옴 band와 함께 정의.
