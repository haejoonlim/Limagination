...
- 지급식 계수 튜닝은 미정G (플탐)
- **잭팟 지급 층: 10층 단위 — 확정 (2026-09-15 D-252 · 시뮬 `jp_A10/B5_naive/B5_rebal` — GK는 5·10·15·20층 명명 배치지만 잭팟은 10의 배수만 · 계수 75 튜닝은 미정G 잔여)**:

| 안 | 지급 근거 | 누적 잭팟 (100층) | S2(12층) 달성률 | 판정 |
|---|---|---|---|---|
| **A. 10층 단위 ×75 (현행)** | 10·20·…·100 — GK 중 주요 게이트 | 2,887,500 | **63%** | **채택** — §3.9표·미정B 계산식 그대로 유효 · 10층 잭팟 1만 마석이 S2 제단 해금(12층) 직전 도달 = "게이트 돌파 → 소환 폭발" 연출 정합 |
| B. 5층 GK 전면 ×75 | GK 5·10·15… 전부 | 11,550,000 (×4.0) | 158% | **기각** — 총량 4배 초과공급, 쿼터 설계 붕괴 |
| C. 5층 GK ×40 (계수 재조정) | 총액 보정 (71,750×40 = 2.87M) | 2,870,000 (−5.6%) | 51% | 예비안 — 곡선은 부드우지만 초반 쿼터 −12%p · 10→20층 잭팟 공백 체감 시 플탐 대안 (시뮬 `jp_every=5, jp_coeff=40`으로 재검증 가능) |
...
> [!note] 판단
> 공백 체감의 본질은 "GK를 이겼는데 아무것도 안 준다"는 것 — lin은 보상이 너무 작아 체감 해소가 안 되고, sq는 잭팟과 같은 f² 곡선으로 후반 잭팟과 스케일이 자연스럽게 이어짐. 총액 +5.5%는 쿼터 90% 달성(23,376→24,654회)에 기여해 §3.8-2 식민섬 의존도도 완화. **플탐 체크 항목 (미정G)**: GK 승리 연출과 소형 보상 표시의 쾌감 검증 · sq 채택 시 §3.9 재계산표·미정B 수치 전체 재산정 필요 (총액이 바뀌므로) |
...
**미정G 잔여 항목 — 플탐 체크리스트 (2026-09-15 총정리 · 확정 시 본 표에서 체크 + 각 섹션 수치 갱신)**
...
|---|---|---|---|---|---|
| G-1 | 마석 지급 계수 (퇴장 ×30·잭팟 ×75) | 30/75 | §3.9 | 쿼터 달성률 추적 (S2 63%·S3 45%·100층 90%) | S2·S3 미달이 체감상 답답하면 상향 — 소형 게이트 보상(sq안) 채택 시 재산정 |
| G-2 | 소형 게이트 보상 채택 여부 (sq안 ×5·f²) | 미채택 (제안) | §3.9 | GK 승리 연출·보상 표시 쾌감 + 5→10층 공백 체감 | 채택 시 §3.9표·미정B 전체 재산정 (총액 +5.5%) |
| G-3 | 추출 수확표 (★1~6 = 1/3/10/30/100/300) | 1/3/10/30/100/300 | §3.1-2 | 50층 ★5 파티 완성 시점 (목표 50 · 시뮬 55) | 60층 이후면 상향 검토 — 캘리브레이션 ±5층 유지 |
| G-4 | 승급 비용 (결정 25~3,000 + 골드) | 현행표 | §3.1-2 | ★5 파티 완성 후 예비 여유 (시뮬 2.3명분) | 예비 0이면 영구사망 재건 불가 — 하향 필요 |
| G-5 | 세금 단가 (+1G/명·클리어) | 1G | §3.2 | 골드 파산 구간 실측 (시뮬 10·25·50층) | 발견 1-2 해결안과 중복 시 단가 조정 대신 지출 쪽 조정 우선 |
...
| G-8 | 유지비 (10G×Lv/섬 하루) | 10G | §3.12 | 예상 런 기간 기준 유지비 총액 (발견 2) | 런 50일 기준 2.8만G — '예상 런 기간' 확정 후 재산정 |
| G-9 | 시작 골드 보완 (1,000~2,000) | 미지급 | 발견 1-2 | 저축형·충동형 플레이어 잔고 비교 | D-251 보완 옵션 — G-6과 세트 판단 |
| G-10 | 확장비 분할 (해금 50% + 3층 후 50%) | 미채택 (추천) | 발견 1-3 | 20·25·50층 잔고 최저 (시뮬 -21,735→-6,735) | 클러스터 완화 효과 체감 시 확정 — 결정로그 신설 |
| G-11 | 채집 수확량·주기 (150/10/8/5 · 24h) | 현행표 | §3.3 | 재료 수급 vs 강화 수요 (광석 병목 포함) | 병목 지속 시 수확량 상향 or 수요 완화 |
| G-12 | 농사 계수 (곡물 30·약초 3·필지 1/동) | 현행표 | §3.3-2 | 식량 재고 곡선 (baseline 6.4만 무아사) | 무아사 유지되면 소모처 확대 or 이벤트 강도 상향 (발견 4) |
...
| G-15 | 사건·계절 강도 (흉작 ×0.7·도적 -10%·주기) | §3.12 가안 | §3.12 | 스트레스 시나리오 대비 실측 (아사 34층은 과함 — S1 기준 튜닝) | 재고 비보유 유저가 즉사하지 않는 선 |
| G-16 | 식량 소모처 확대 여부 | 없음 | 발견 4 | baseline 재고 곡선 (6.4만 누적) | 소모처 신설 or G-12 생산 하향 중 택일 |
