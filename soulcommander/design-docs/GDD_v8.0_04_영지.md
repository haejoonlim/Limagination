...
| # | 원칙 | 이유 |
|---|---|---|
| 1 | 단일 국고(골드) — 신화폐 없음 (**예외 1건: 전당의 은혜** — §3.12-2, 2026-09-15 확정) | 관리 피로 최소 (식량·재료·영혼석은 Consumable) |
...
| 추출소 | 보유 영웅 → 성혼 결정+기억 조각 (자발 환원만 · 영원한 잠 — §3.1-2) | 30층 | 1 |
...
- **등급 게이트 (05 §6 7차)**: 명장(티어7)은 ★5+ 영웅만 보유 가능 — 명장 운영자는 소환에서도 극희소(30k 중 3명)라 **훈련 상승(티어 성장) 확정 시 주 확보 경로**가 됨
- 인력 = 배치 주민 (영웅 겸직 — 원칙 #2) · 표의 '1' = 겸직 영웅 1명
- 건설비는 해금 시점 누적 골드 대비 10~20%선 (가안) — 전술소 1,000G 확정, 나머지 플탐 전 확정

### 3.1-2 추출·승급 (성혼 결정 — 2026-09-15 2차 설계: 50층 ★5 파티 캘리브레이션)

> [!info] 이 서브섹션은?
> 추출소 산출물 = **성혼 결정**(승급 재료 — 구 '영혼 결정'에서 명칭 변경: 영혼석과 혼동 방지 · 성 = ★ 등급 어원). 합성소의 승급 재료·비용을 동시 정의 — §3.11 "합성강화"·§3.10 합성비 할인의 빈 참조 해소. 결정은 재료 분류 (원칙 #1 Consumable — 화폐 신설 아님).

- **성혼 결정**: 보유 영웅 추출로만 획득 (유일 수급처) · 합성소 영웅 승급 소모
- 추출 수확 (가안 — 미정G):

| 등급 | ★1 | ★2 | ★3 | ★4 | ★5 | ★6 |
|---|---|---|---|---|---|---|
| 성혼 결정 | 1 | 3 | 10 | 30 | 100 | 300 |

- 레벨 보정: ×(1+(Lv−1)×0.5%) — Lv100 ×1.50 (훈련 후 추출이 이득 — 훈련장 가치 연동 · Lv 상한 100 — 00 #14)
- 기억 조각 동시 지급 — **★4+ 전용** (★4 5 · ★5 20 · ★6 50): 용도 = ★7 초월(미정C) 재료 후보. ★1~3에도 지급 시 50층까지 5,000+ 적립돼 초월 수량 설계 무의미 — ★4+ 제한 (50층 적립 ≈540개 · 100층 ≈3,740개)
- 추출 규칙: 자발만 (강제 없음) · 배치·파티 편성 중 추출 불가 (해제 후 가능) · 추출 영웅 = **영원한 잠** — 동일 개체 재등장 불가 (이름 재추첨 05 §8 구조) · 영웅의 전당(전사자) 등재 대상 아님 · ★5+ 추출 시 2차 확인 팝업 (v7.12 승계)
- 승급 비용 (합성소 — 가안 · 미정G):

| 승급 | 성혼 결정 | 골드 (합성비 — §3.10 할인 적용 대상) |
|---|---|---|
| ★1→★2 | 25 | 50 |
| ★2→★3 | 100 | 200 |
| ★3→★4 | 375 | 750 |
| ★4→★5 | 1,200 | 2,400 |
| ★5→★6 | 3,000 | 5,000 |
| ★6→★7 초월 | 미정C — 기억 조각 + 결정 (다음 결정) | 8,000 |

- **캘리브레이션 (50층 = ★5 파티 1개 — 00 미정B 연동)**: 50층 누적 소환 3,467회 → 추출 기대 결정 **10,230개** (★1 2,428 + ★2 2,080 + ★3 2,427 + ★4 3,016 + ★5 173 + ★6 104 — 레벨 보정 전). ★1→★5 승급 = 1,700결정+3,400G/영웅 → 파티 5인 = **8,500결정 + 17,000G = 수급의 83%**. 뽑기 기대 ★5 1.73 + 승급 3.3명 → **50층에서 ★5 파티 1개 완성선** · 전량 추출 최적화 시 잔여 ≈4,670결정 = 추가 ★5 2.7명 (파티+예비 2~3 · 통상 플레이는 파티 1개선 수렴). 더 빡세게 하려면 ★4→★5 1,200→1,700 (파티+1명 압축) — 플탐 판단 (미정G)
- 100층 누적 결정 ≈ 68,965개 — 50층 이후 목표: **★6 파티** (5×3,000 = 15,000결정+25,000G) 또는 제2 ★5 파티 + 초월 추진 — 선택과집중 유지
- 골드 규모: 승급 파티 17,000G = 50층 던전 누적 수입(127,500G)의 13% — **결정이 주 게이트, 골드는 부 게이트** (건설·업글·유지비와 경쟁)
- 소환→추출 순환 시 영혼석 0 반환 — 순환 루프 없음 (손실만) · **소환 = 풀 확장, 합성소 = 육성** 분업
...
- 소환 경제 정합 (2026-09-15 재계산 — 00 #19 재계산표 연동): 영웅 수요 36 = 티켓 4,680석 = **18회 소환분** — S4 해금(50층) 누적 티켓 3,467회의 **0.5%**·100층 누적 23,376회의 **0.08%**. 소환은 등급 경쟁(★5~★6 추구)이 소비 주체 — 배치 수요는 소환 경제의 제약이 아님. 참고: E[★5] = 36×0.05% = 0.018장 — 소환 36회 전부 ★1~★3 기대, 배치 효율 +20%(적성 일치)는 적성 롤(05) 분포에 의존. 소환 소비·재화 설계와 무충돌
...
- 배율형(훈련·연구·연금효율): ×(1+0.5×(Lv-1)) → L7 ×4.0 · **최종 효율 = 시설Lv배율 × 운영자 적성 티어배율(4/5/6/7 — 05 §6)** 곱연산
  ...
  - 명장(티어7)은 ★5+ 영웅만 보유 — 소환 30k 중 3명 수준의 극희소 운영자 (티어 성장 시스템 확정 시 주 확보 경로)
...
| 영혼석 | 던전 퇴장 층×30·게이트잭팟 · 무료 10회 | 소환 130석/회(티켓) — 합성·승급은 성혼 결정·골드로 이관 (§3.1-2) |
| 식량 | 채집×tier · 농사 · 식당 가공 | 인구 1인1층 1 |
| 재료 | 채집·농사 · 추출소(성혼 결정) | 가공(요리·포션) · 영웅 승급(합성소 — §3.1-2) · 장비 강화(광석) · 매각 |
...
- **재화**: 전당의 은혜 — 기존 재화 체계(골드·영혼석·식량·재료) 외 별도 신설 · 원칙 #1 예외 (2026-09-15 결정)
...
- [ ] 적성 티어 곡선 밸런스 (가안): ×1.10/×1.30/×1.65/×2.30 — 시설Lv배율과 곱연산 시 후반 폭발 여지 (L7×명장 ×9.2) → 플탐에서 상한 검토 · 명장 운영자 확보 경로 (티어 성장) 설계 대기
